...
const AnsibleVariablesTableRows = ({ assignedRoles, resourceErrors }) =>
  assignedRoles.map(role => {
    ...
    return overrideKeys.map((lookupKey) => {
      ...
      return (
        <AnsibleVariablesTableRow key={lookupKey.id} role={role} lookupKey={lookupKey} firstKey={firstKey} resourceError={resourceError}/>
      )}
    );
  });

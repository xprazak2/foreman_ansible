class DataAnsibleRolesController < ::Api::V2::BaseController
  def resource_name(resource = 'AnsibleRole')
    super resource
  end

  def index
    @ansible_roles = resource_scope_for_index(:permission => :view_ansible_roles).includes(:ansible_variables)
    overrides = calculate_variable_overrides(params[:resource_id],
                                             params[:resource_name],
                                             AnsibleVariable.where(:ansible_role_id => @ansible_roles.pluck(:id)))
  ...
  def calculate_variable_overrides(host_or_hg_id, resource_name, ansible_variables)
    return {} unless !host_or_hg_id || !resource_name
